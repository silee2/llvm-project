...
gpu.func @load_gather_ui64_src_constant_offset(%src: ui64) {
  ...
  // CHECK: %[[VAR5:.*]] = arith.addi %[[VAR1]], %[[VAR3]] : i64
  ...
  // CHECK: %[[VAR7:.*]] = scf.if %[[VAR4]] -> (vector<2xf32>) {
  // CHECK:      %[[VAR8:.*]] = llvm.load %[[VAR6]] {cache_control = #xevm.load_cache_control<L1c_L2uc_L3uc>} : !llvm.ptr<1> -> vector<2xf32>
  // CHECK:      scf.yield %[[VAR8]] : vector<2xf32>
  // CHECK:    } else {
...
gpu.func @load_gather_memref_src_constant_offset(%src: memref<256xf32>) {
  ...
  // CHECK: %[[VAR6:.*]] = scf.if %[[VAR2]] -> (f32) {
  // CHECK:      %[[VAR7:.*]] = llvm.load %[[VAR5]] {cache_control = #xevm.load_cache_control<L1c_L2uc_L3uc>} : !llvm.ptr<1> -> vector<1xf32>
  // CHECK:      %[[VAR8:.*]] = vector.extract %[[VAR7]][0] : f32 from vector<1xf32>
  // CHECK:      scf.yield %[[VAR8]] : f32
  // CHECK:    } else {
  // CHECK:      %[[CST_1:.*]] = arith.constant dense<0.000000e+00> : vector<1xf32>
  // CHECK:      %[[VAR7:.*]] = vector.extract %[[CST_1:.*]][0] : f32 from vector<1xf32>
  // CHECK:      scf.yield %[[VAR7]] : f32
...
gpu.func @load_gather_memref_src_value_offset(%src: memref<256xf16>, %offset: vector<1xindex>) {
  ...
  // CHECK: %[[VAR5:.*]] = llvm.inttoptr %[[VAR4]] : i64 to !llvm.ptr<1>
  // CHECK: %[[VAR6:.*]] = scf.if %[[VAR2]] -> (vector<8xf16>) {
  // CHECK:      %[[VAR7:.*]] = llvm.load %[[VAR5]] {cache_control = #xevm.load_cache_control<L1c_L2uc_L3uc>} : !llvm.ptr<1> -> vector<8xf16>
  // CHECK:      scf.yield %[[VAR7]] : vector<8xf16>
  // CHECK:    } else {
...
gpu.func @load_gather_memref_src_load_offset(%src: memref<256xf16>, %offset1: vector<1xindex>, %offset2: vector<1xindex>) {
  ...
  // CHECK: %[[VAR9:.*]] = scf.if %[[VAR4]] -> (vector<8xf16>) {
  // CHECK:      %[[VAR10:.*]] = llvm.load %[[VAR8]] {cache_control = #xevm.load_cache_control<L1c_L2uc_L3uc>} : !llvm.ptr<1> -> vector<8xf16>
  // CHECK:      scf.yield %[[VAR10]] : vector<8xf16>
  // CHECK:    } else {
  // CHECK:      %[[CST_0:.*]] = arith.constant dense<0.000000e+00> : vector<8xf16>
  // CHECK:      scf.yield %[[CST_0]] : vector<8xf16>
...
gpu.func @store_scatter_ui64_src_constant_offset(%src: ui64) {
  ...
  // CHECK: %[[VAR6:.*]] = llvm.inttoptr %[[VAR5]] : i64 to !llvm.ptr<1>
  // CHECK:    scf.if %[[VAR4]] {
  // CHECK:      llvm.store %[[CST_1]], %[[VAR6]] {cache_control = #xevm.store_cache_control<L1wb_L2uc_L3uc>} : vector<2xf32>, !llvm.ptr<1>
...
gpu.func @store_scatter_memref_src_constant_offset(%src: memref<256xf32>) {
  ...
  // CHECK: %[[VAR5:.*]] = llvm.inttoptr %[[VAR4]] : i64 to !llvm.ptr<1>
  // CHECK: scf.if %[[VAR2]] {
  // CHECK:      llvm.store %[[CST_1]], %[[VAR5]] {cache_control = #xevm.store_cache_control<L1wb_L2uc_L3uc>} : vector<2xf16>, !llvm.ptr<1>
