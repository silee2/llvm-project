...
#a = #xegpu.layout<sg_layout = [2, 2], sg_data = [16, 2048], inst_data = [8, 64], lane_layout = [1, 16], lane_data = [1, 1]>
#b_packed = #xegpu.layout<sg_layout = [2, 2], sg_data = [1024, 16], inst_data = [32, 16], lane_layout = [1, 16], lane_data = [4, 1]>
#b = #xegpu.layout<sg_layout = [2, 2], sg_data = [2048, 16], inst_data = [64, 16], lane_layout = [1, 16], lane_data = [8, 1]>
...
#a_scale = #xegpu.layout<sg_layout = [2, 2], sg_data = [16, 64], inst_data = [16, 32], lane_layout = [16, 1], lane_data = [1, 1]>
#b_scale = #xegpu.layout<sg_layout = [2, 2], sg_data = [64, 16], inst_data = [32, 16], lane_layout = [1, 16], lane_data = [1, 1]>
// Note: scales for dpas_mx needs separate layouts with inst_data to match HW constraint. Otherwise dpas_mx is not unrolled
#dpas_a_scale = #xegpu.layout<sg_layout = [2, 2], sg_data = [16, 64], inst_data = [8, 2], lane_layout = [8, 1], lane_data = [1, 1]>
#dpas_b_scale = #xegpu.layout<sg_layout = [2, 2], sg_data = [64, 16], inst_data = [2, 16], lane_layout = [1, 16], lane_data = [1, 1]>
...
module @gemm attributes {gpu.container_module} {
  gpu.module @kernel {
    gpu.func @gemm_mxfp(%arg0: memref<256x4096xf4E2M1FN>, %arg1: memref<2048x256xi8>, %arg2: memref<256x128xf8E8M0FNU>, %arg3: memref<128x256xf8E8M0FNU>, %arg4: memref<256x256xf32>) kernel {
      ...
      %a_tdesc = xegpu.create_nd_tdesc %arg0 : memref<256x4096xf4E2M1FN> -> !xegpu.tensor_desc<32x4096xf4E2M1FN>
      %bp_tdesc = xegpu.create_nd_tdesc %arg1 : memref<2048x256xi8> -> !xegpu.tensor_desc<2048x32xi8>
      %a_scale_tdesc = xegpu.create_nd_tdesc %arg2 : memref<256x128xf8E8M0FNU> -> !xegpu.tensor_desc<32x128xf8E8M0FNU>
      %b_scale_tdesc = xegpu.create_nd_tdesc %arg3 : memref<128x256xf8E8M0FNU> -> !xegpu.tensor_desc<128x32xf8E8M0FNU>
      ...
      %c_init = xegpu.load_nd %cd_tdesc[%m, %n] {layout = #c}: !xegpu.tensor_desc<32x32xf32, #c> -> vector<32x32xf32>
        ...
        // load_nd with offset
        %a = xegpu.load_nd %a_tdesc[%m, %c0] {layout = #a}: !xegpu.tensor_desc<32x4096xf4E2M1FN> -> vector<32x4096xf4E2M1FN>
        %bp = xegpu.load_nd %bp_tdesc[%c0, %n] {layout = #b_packed}: !xegpu.tensor_desc<2048x32xi8> -> vector<2048x32xi8>
        ...
        %scale_a = xegpu.load_nd %a_scale_tdesc[%m, %c0] {layout = #a_scale}: !xegpu.tensor_desc<32x128xf8E8M0FNU> -> vector<32x128xf8E8M0FNU>

        %scale_b = xegpu.load_nd %b_scale_tdesc[%c0, %n] {layout = #b_scale}: !xegpu.tensor_desc<128x32xf8E8M0FNU> -> vector<128x32xf8E8M0FNU>
