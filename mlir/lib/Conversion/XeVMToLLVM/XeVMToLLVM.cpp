...
getCacheControlMetadata(ConversionPatternRewriter &rewriter, OpType op,
  ...
  if constexpr (isLoad) {
    if (op.getCacheControlOrDefault() == LoadCacheControl::DEFAULT)
      return {};
  } else {
    if (op.getCacheControlOrDefault() == StoreCacheControl::DEFAULT)
      return {};
  }
