...
extern "C" SYCL_RUNTIME_EXPORT void
mgpuMemset32(void *dst, unsigned int value, size_t count, sycl::queue *queue) {
  std::vector<unsigned int> data(count, value);
  catchAll(
      [&]() { queue->memcpy(dst, (void *)data.data(), count * 4).wait(); });
}

extern "C" SYCL_RUNTIME_EXPORT void mgpuMemset16(void *dst,
                                                 unsigned short value,
                                                 size_t count,
                                                 sycl::queue *queue) {
  std::vector<unsigned int> data(count, value);
  catchAll(
      [&]() { queue->memcpy(dst, (void *)data.data(), count * 2).wait(); });
}
